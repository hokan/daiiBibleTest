                ...
                if (selectedBook) { // 確保有選擇書卷
                    ...
                    chapterNumbers.forEach(chapterNumber => { // 迴圈遍历章節數
                        ...
                        chapterButton.addEventListener('click', function() {
                                ...
                                chapterVerses.forEach((verse, index) => {
                                    ...
                                    verseElement.addEventListener("click", function () {
                                        ...
                                        console.log('高亮設定完成：', {
                                            ...
                                            章節: currentChapter,
                                            經文: this.textContent,
                                            DOM狀態: this.classList.contains('selected')
                                        });
                                    ...
                                    // ===== 修正後的長按複製功能 (最終版) =====
                                    // 修改 touchstart 事件處理函式開頭
                                    verseElement.addEventListener("touchstart", function(e) {
                                        console.log('觸發 touchstart 事件', this.textContent); // 新增事件觸發日誌
                                        
                                        // 四維驗證 (新增 timestamp 檢查)
                                        const isValid = (
                                            selectedVerseElement !== null &&
                                            selectedVerseElement === this &&
                                            this.classList.contains('selected') &&
                                            Date.now() - this.lastClickTime < 5000 // 5秒內有效
                                        );
                                        
                                        if (!isValid) {
                                            console.log('🚨 非法觸控', {
                                                element: this.textContent,
                                                selected: selectedVerseElement?.textContent,
                                                isSame: this === selectedVerseElement,
                                                hasClass: this.classList.contains('selected'),
                                                timeDiff: Date.now() - (this.lastClickTime || 0)
                                            });    
                                            return;
                                        }
                            ...
                            setTimeout(() => {
                                bibleTextDiv.querySelectorAll('p').forEach(checkEventListeners);
                            }, 1000);
                        });
